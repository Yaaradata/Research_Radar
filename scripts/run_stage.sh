...
Stages (run in this order):
  ...
  6) classify    Scoring v3 — domain/audience/paper_kind on full eligible pool (PAID — needs --allow-paid)
                 Optional --from/--until (ISO YYYY-MM-DD) bound ci.published_at; --until is inclusive.
 ...
 13) topics           Hierarchical topic tags + key-claim extraction (PAID — needs
                 ...
                 Optional --from/--until (ISO YYYY-MM-DD) bound ci.published_at;
                 --until is inclusive.
...
Examples:
  ...
  ./scripts/run_stage.sh screen --from 2026-09-04 --until 2026-09-04 --dry-run
  ./scripts/run_stage.sh classify --from 2026-09-04 --until 2026-09-05 --dry-run
