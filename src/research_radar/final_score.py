...
def upsert_final_score(conn, *, content_id, score_version, assessment_id, result, weights):
    conn.execute(
        ...
        INSERT INTO research_radar.content_final_scores(
            ...
            final_score, assessment_id, components, provenance, computed_at)
        ...
        (
            ...
            json.dumps(
                {
                    "method": "semantic_core_plus_verified_signals",
...
def load_top(conn, *, profile=None, top=20, since_days=None):
...
def render_markdown(rows, *, profile, since_days=None, corpus_n=None) -> str:
    score_version, weights = resolve_profile(profile)
